-- Create an admin view that shows all tools regardless of approval status
DROP VIEW IF EXISTS public.admin_tools_view;

CREATE VIEW public.admin_tools_view AS
WITH criteria_array AS (
    SELECT 
        t.id AS tool_id,
        jsonb_agg(
            jsonb_build_object(
                'id', c.id,
                'name', c.name,
                'ranking', COALESCE(ct.ranking, 0),
                'description', COALESCE(ct.description, '')
            )
            ORDER BY c.name
        ) FILTER (WHERE c.id IS NOT NULL) AS criteria
    FROM tools t
    CROSS JOIN (
        SELECT * FROM criteria WHERE active_on <= now()
    ) c
    LEFT JOIN criteria_tools ct ON t.id = ct.tool_id AND ct.criteria_id = c.id
    WHERE t.type = 'application'
    GROUP BY t.id
),
tags_array AS (
    SELECT 
        t.id AS tool_id,
        jsonb_agg(
            jsonb_build_object(
                'id', tg.id,
                'name', tg.name,
                'type', tt.name
            )
            ORDER BY tg.name
        ) FILTER (WHERE tg.id IS NOT NULL) AS tags
    FROM tools t
    LEFT JOIN tag_tools tt_link ON t.id = tt_link.tool_id
    LEFT JOIN tags tg ON tt_link.tag_id = tg.id
    LEFT JOIN tag_type tt ON tg.tag_type_id = tt.id
    WHERE t.type = 'application'
    GROUP BY t.id
)
SELECT 
    t.id,
    t.name,
    t.type,
    t.created_by,
    COALESCE(ca.criteria, '[]'::jsonb) AS criteria,
    COALESCE(ta.tags, '[]'::jsonb) AS tags,
    t.created_on,
    t.updated_at,
    t.submitted_at,
    t.approved_at,
    t.submission_status
FROM tools t
LEFT JOIN criteria_array ca ON t.id = ca.tool_id
LEFT JOIN tags_array ta ON t.id = ta.tool_id
WHERE t.type = 'application';

-- Ensure the regular tools_view only shows approved tools
DROP VIEW IF EXISTS public.tools_view;

CREATE VIEW public.tools_view AS
WITH criteria_array AS (
    SELECT 
        t.id AS tool_id,
        jsonb_agg(
            jsonb_build_object(
                'id', c.id,
                'name', c.name,
                'ranking', COALESCE(ct.ranking, 0),
                'description', COALESCE(ct.description, '')
            )
            ORDER BY c.name
        ) FILTER (WHERE c.id IS NOT NULL) AS criteria
    FROM tools t
    CROSS JOIN (
        SELECT * FROM criteria WHERE active_on <= now()
    ) c
    LEFT JOIN criteria_tools ct ON t.id = ct.tool_id AND ct.criteria_id = c.id
    WHERE 
        t.type = 'application' AND
        (t.submission_status = 'approved' OR t.submission_status IS NULL)
    GROUP BY t.id
),
tags_array AS (
    SELECT 
        t.id AS tool_id,
        jsonb_agg(
            jsonb_build_object(
                'id', tg.id,
                'name', tg.name,
                'type', tt.name
            )
            ORDER BY tg.name
        ) FILTER (WHERE tg.id IS NOT NULL) AS tags
    FROM tools t
    LEFT JOIN tag_tools tt_link ON t.id = tt_link.tool_id
    LEFT JOIN tags tg ON tt_link.tag_id = tg.id
    LEFT JOIN tag_type tt ON tg.tag_type_id = tt.id
    WHERE 
        t.type = 'application' AND
        (t.submission_status = 'approved' OR t.submission_status IS NULL)
    GROUP BY t.id
)
SELECT 
    t.id,
    t.name,
    t.type,
    t.created_by,
    COALESCE(ca.criteria, '[]'::jsonb) AS criteria,
    COALESCE(ta.tags, '[]'::jsonb) AS tags,
    t.created_on,
    t.updated_at,
    t.submitted_at,
    t.approved_at,
    t.submission_status
FROM tools t
LEFT JOIN criteria_array ca ON t.id = ca.tool_id
LEFT JOIN tags_array ta ON t.id = ta.tool_id
WHERE 
    t.type = 'application' AND
    (t.submission_status = 'approved' OR t.submission_status IS NULL);

-- Give public access to both views
GRANT SELECT ON tools_view TO PUBLIC;
GRANT SELECT ON admin_tools_view TO PUBLIC;

-- Make sure status filters are properly enforced with policies
DO $$ 
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_catalog.pg_policies 
    WHERE tablename = 'tools' AND policyname = 'Admin can view all tools'
  ) THEN
    EXECUTE 'CREATE POLICY "Admin can view all tools" ON tools 
             FOR SELECT TO authenticated 
             USING (is_admin() OR submission_status = ''approved'' OR submission_status IS NULL)';
  END IF;
END $$;