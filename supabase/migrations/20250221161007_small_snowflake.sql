/*
  # Add Combined Tools View

  1. New View
    - Creates a view that combines both application tools and approved user submissions
    - Includes all tool metadata, criteria, and tags
    - Ensures proper access control

  2. Security
    - Creates the view with security definer option
    - Adds policy for public read access
*/

-- Create a view that combines application tools and approved user submissions
...
WITH criteria_array AS (
    ...
    FROM (
        ...
        UNION ALL
        SELECT id, name, type FROM tool_submissions WHERE status = 'approved'
    ) t
