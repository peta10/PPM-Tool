/*
  ...
  1. Changes
    - Simplify view to focus on core functionality
    - Ensure proper handling of criteria and tags
    - Add better error handling for null values

  2. Security
    - Maintain existing RLS policies
    - Keep security barrier
*/

-- Drop and recreate the view with simplified logic
...
WITH criteria_array AS (
    SELECT 
        ...
        jsonb_agg(
            ...
            ORDER BY c.name
        ) FILTER (WHERE c.id IS NOT NULL AND ct.ranking IS NOT NULL) AS criteria
    FROM tools t
